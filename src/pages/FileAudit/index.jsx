...
export default function FileAudit(props) {
    ...
    const [commentId, setCommentId] = useState("")  // 批注id
    // 编辑器内容
    ...
    useEffect(() => {
        api.getArticleTitleAndContent(id).then(res => {
            setHtml(`摘要：${res.data.fileSummary}\n\n${res.data.fileString}`)
    ...
    function customParseLinkUrl(url) {                // JS 语法
        ...
        return `${SELF_ADDRESS}file-audit/${commentId}`
        // if (url.indexOf('http') !== 0) {
        //     return `http://${url}`
        // }
        // return url
            ...
            <Row>
                <Col sm={14}>
                    <div style={{marginTop: "30px", paddingRight: "20px", borderRight: "dashed 1px gray"}}>
                        ...
                        <Button onClick={() => {
                            api.uploadNote(id, html, commentId).then(res => {
                                tips(res)
                            })
